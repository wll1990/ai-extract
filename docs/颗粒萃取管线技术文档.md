...
| 验证 batch size | 10 | verifyGrains:677 |
...
格式	状态	适合销售素材？
.txt .md	✅ 直接读取	✅ 转录文本、笔记
.docx	✅ python-docx 提取	✅ 最常见
.pdf	✅ PyPDF2 提取	✅ 合同/方案
.ppt .pptx	✅ 提取文本框	✅ 销售提案/培训
.xls .xlsx .csv	✅ pandas 读取	⚠️ 客户清单/数据表
.html .htm	✅ BeautifulSoup	⚠️ 网页存档
.png .jpg .jpeg	⚠️ needs_manual	❌ 需人工补充
.mp3 .wav	⚠️ needs_manual	❌ 需 ASR 转写
.m4a	❌ 没实现！	—



单份素材（夏宇 45min 访谈）Token 账单
萃取管线
层	调用次数	每次输入	每次输出	小计入	小计出
L0 情境标注	1	~4,500	~100	4,500	100
L4 提取 full	1	~3,000	~500	3,000	500
L4 提取 short	20	~2,000	0~500	40,000	4,000
L5 场景归类	1	~1,500	~150	1,500	150
L6 验证	2	~3,000	~1,500	6,000	3,000
L9 FAQ	1	~6,000	~500	6,000	500
L10 叙事	1	~2,000	~1,500	2,000	1,500
L11 画像(异步)	1	~1,500	~500	1,500	500
管线合计	28	—	—	64,500	10,250
报告生成（额外）
步骤	输入	输出	小计入	小计出
Part1	~6,000	~1,200	6,000	1,200
Part2	~2,500	~1,200	2,500	1,200
Part3	~2,500	~800	2,500	800
报告合计	—	—	11,000	3,200
总计

一份 45 分钟访谈 = 75,500 输入 + 13,450 输出 ≈ 89K token
DeepSeek 定价: $0.14/1M in + $0.28/1M out
成本: (75,500 × 0.14 + 13,450 × 0.28) / 1,000,000 = $0.014
约 1.4 美分。3 小时访谈约 $0.05。不贵。

但真正的问题是 调用次数，不是 token 量。28 次调用、28 次网络往返。瓶颈在这里：


L4 提取 21 次调用 = 75% 的调用量 = 90% 的耗时
每个 chunk 要单独把 prompt 指令（~300 字 role + 约束 + 输出格式）发给模型。21 个 chunk 就是 21 次重复发送这些指令。

可以优化的不是 token 量，是指令冗余。但 short prompt 已经是精简版了（~200 字），再精简就回到"规则同上"的问题。

当前设计在 token 成本上已经合理。1.4 美分一份素材，在这个商业场景下不需要优化。



对于 B 端客户——园区花 5 万买一套系统，之前只能萃取出销冠 30% 的经验。员工跟 AI 对练，问 10 个问题有 7 个答不上来或者答得笼统。用两周就弃了，"跟 ChatGPT 差不多"。现在 31 个颗粒，转介绍、微信维护、政府渠道获客这些隐性知识全出来了，AI 分身从"听起来有道理"变成"拿着就能用"。

对于竞品——大模型能力是平的，谁都能调 DeepSeek。但颗粒质量不是平的。我们改了 prompt、改了分块策略、改了验证阈值，这一套调优的 know-how 是项目里磨出来的，不是看论文能抄的。竞品拿同样一份访谈，用通用管线跑出来的还是 9 个颗粒，他不知道为什么少，也不知道怎么多。

对于数据飞轮——每个销冠访谈多产出 22 个颗粒，10 个销冠就是 220 个，100 个销冠就是 2200 个。这个差距会越滚越大。颗粒越密，RAG 检索命中率越高，用户越觉得 AI 分身"懂行"，续费率越高，素材越多，颗粒越多——飞轮转起来了。