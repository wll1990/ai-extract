        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6">
          <div className="mx-auto max-w-[720px] space-y-4">
            ...
            {state.messages.map((msg) => {
              ...
              return (
                <InterviewMessageBubble key={msg.id}
                  role={msg.role}
                  content={msg.content}
                  depth={msg.depth}
                ...
                <div className="mt-6 rounded-2xl bg-surface-2 p-6 sm:p-8 shadow-lg"
                  ...
                  {isH5 && platformHost && (
                    <p className="mt-4 text-center text-xs text-muted-foreground">
                      💡 完整报告和审核进度请访问{' '}
                      <span className="text-primary font-medium select-all">{platformHost}</span>
                    </p>
                  )}
                </div>
              );
              ...
              <div className="mt-6 rounded-2xl bg-surface-2 p-6 text-center shadow-lg">
                ...
                {isH5 && platformHost && (
                  <p className="mb-4 text-xs text-muted-foreground">
                    完整报告请访问 <span className="text-primary font-medium select-all">{platformHost}</span>
                  </p>
                )}
