...
export function ChatView({ skill, enterprise, initialQuestion, initialMode }: ChatViewProps) {
  const router = useRouter();
  const initMode = (initialMode as Mode) || (enterprise ? 'qa' : 'talk');
  ...
  const chat = useChat({ skillId: skill.id, ownerName, enterprise });

  // Auto-send: 名片页带问题进来 → 自动发送 → 清除 URL 参数防刷新重发
  useEffect(() => {
    if (initialQuestion && skill.id) {
      const timer = setTimeout(() => {
        chat.sendMessage(initialQuestion, initialMode || 'talk');
        router.replace(`/chat/${skill.id}`);
      }, 150);
      return () => clearTimeout(timer);
    }
  }, [initialQuestion, skill.id]);
